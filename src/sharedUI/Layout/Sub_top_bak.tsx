...
const SubTop: React.FC<subTopProps> = ({
  ...
  const patchInfoBox = useMemo(
    () => ({
      ...
      infoText: (
        <>
          ㅇㅇ ㅇㅇㅇㅇ <span className="text-blue-800">올해 기준 점수</span>
          입니다.
        </>
      ),
      ...infoBox,
      ...
      <div className="mt-8 md:mt-10 relative">
        <div className="flex flex-wrap justify-between ">
          <div>
            {patchSubTitle?.visible && (
              <>
                <Title as="h2" title={patchSubTitle.text} sub={patchSubDate.text}>
                  {patchSubTitle.text}{patchSubDate.text}
                </Title>
              </>
            )}
          </div>
                ...
                {flagItems.map(
                  (item, index) =>
                    item.flagType && (
                      <p
                        key={index}
                        className={`inline-block px-4 md:px-6 py-2 text-3xs md:text-s text-center ${item.textColor} ${item.textBg} rounded-full`}
                      >
                        {item.text}
                      </p>
                    )
        ...
        {patchSubText?.visible && (
          <div className="mt-4 w-full">
            <p className="text-3xs md:text-s text-gray-400">
              {patchSubText.text}
            </p>
          </div>
        )}
