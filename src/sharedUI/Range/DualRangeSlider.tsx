...
export interface DualRangeSliderProps
  extends VariantProps<typeof DualRangeSliderVariants> {
  type?: rangeType;
  min: number;
  max: number;
  deadZones?: {
  ...
  step: number;
  tick?: { show?: boolean; length: number };
...
export const DualRangeSlider: React.FC<DualRangeSliderProps> = ({
  ...
  const isDeadZone = (val: number) =>
    deadZones.some((dzValue) => val >= dzValue.from && val <= dzValue.to);

  const commitMin = (v: number) => {
    if (isDeadZone(v)) return;
  ...
  for (let i = 0; i <= tick.length; i += 1) {
    const tickPosition = ((i - 0) / (tick.length - 0)) * 100;
  ...
  return (
    <div className={`w-full`}>
      <div className={`${cn(className, addClass, tick.show && 'mb-6')}`}>
        <div
          ref={trackRef}
          onPointerDownCapture={onTrackPointerDownCapture}
          className={`${cn(
            'absolute top-0 bottom-0 w-full rounded-lg cursor-pointer',
        ...
        >
          ...
          >
            {type === 'recom' && (
              <>
                <div className="absolute left-0 -ml-2 w-[1.125rem] h-3 sm:h-4 sm:w-5 -top-6 sm:-top-8">
                  <svg
                  ...
                  >
                    <path
                      d="M1.34031 0C0.362638 0 -0.204715 1.10648 0.365886 1.90036L9.52558 14.6443C10.0044 15.3105 10.9956 15.3105 11.4744 14.6443L20.6341 1.90037C21.2047 1.10649 20.6374 0 19.6597 0H1.34031Z"
                      fill="#A3C7D7"
                    />
                  </svg>
                  <span className="sr-only">min value</span>
                </div>
                <div className="absolute right-0 w-[1.125rem] h-3 sm:h-4 sm:w-5 -mr-2 -top-6 sm:-top-8">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="100%"
                    height="100%"
                  ...
                  <span className="sr-only">max value</span>
                </div>
              </>
            )}
          </div>
