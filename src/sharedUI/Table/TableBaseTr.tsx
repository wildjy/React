...
export const TableBaseTr: React.FC<TableBaseProps> = ({ addClass, thW = 'w-1/3', tdW = 'w-2/3', datas }) => {
  ...
  const hasRowSpan = datas.some((row) => row.children.some((cell) => cell.thRow));
      ...
      <TableTr addClass={`${cn('tableBase', addClass)}`}>
        ...
        <TableTr.Thead thW={thW}>
          {datas.map((row, rowIndex) => {
            const visibleCells = row.children.filter((cell) => cell.title);
              ...
              <tr key={rowIndex}>
                {visibleCells.map((obj, i) => (
                  <th
                    ...
                    colSpan={obj.thCol ?? 1}
                    rowSpan={obj.thRow ?? 1}
                    className={clsx(`${obj.hide ? 'hide' : ''}`, hasRowSpan && 'py-3', {
              ...
              <tr key={index}>
                {visibleCells.map((obj, i) => (
                  <td
                    ...
                    colSpan={obj.col ?? 1}
                    rowSpan={obj.row ?? 1}
                    className={`${obj.hide ? 'hide' : ''} ${cn('px-0 lg:px-4', {
