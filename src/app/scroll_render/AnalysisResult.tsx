...
export const AnalysisResult: React.FC<AnalysisResultProps> = ({ Analysis, name, myScore, type = 'type1', score }) => {
  const defaultType = {
    type1: { label: '안정지원', txtColor: 'text-[#8393D6]', color1: '#8393D6', color2: '#84D8F0' },
    type2: { label: '적정지원', txtColor: 'text-[#54AEC8]', color1: '#54AEC8', color2: '#FFA8A8' },
    type3: { label: '소신지원', txtColor: 'text-[#99CC33]', color1: '#99CC33', color2: '#91A7FF' },
    type4: { label: '위험지원', txtColor: 'text-[#B2B1B1]', color1: '#B2B1B1', color2: '#B2F2BB' },
    type5: { label: '매우위험', txtColor: 'text-[#7D7D7D]', color1: '#7D7D7D', color2: '#B2F2BB' },
  };
        ...
        <div className="mx-5 md:mx-7 w-[5rem] h-[5rem] sm:w-[6rem] sm:h-[6rem] md:w-[5rem] md:h-[5rem] xl:w-[7.5rem] xl:h-[7.5rem] relative">
          <svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 96 96" fill="none">
            ...
            <defs>
              ...
              >
                <stop offset="0.0121951" stop-color={types.color2} stop-opacity="0.5" />
                <stop offset="0.865" stop-color={types.color1} />
              </radialGradient>
            </defs>
          </svg>
          <p
            ...
            text-white text-xs sm:text-base xl:text-xl
            `}
          >
            {types.label}
          </p>
        ...
        <div className="lg:mx-7 grow">
          <ul className="flex-wrap md:flex text-2xs sm:text-sm md:text-base xl:text-lg">
            {Analysis.map((item, index) => (
              <li
                key={index}
                className={`pt-2 first:pt-0 md:w-1/4 flex flex-wrap justify-between items-center ${
