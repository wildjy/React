...
export function HandbookLayout({ children }: HandbookLayoutProps) {
  const [sidebarOpen, setSidebarOpen] = useState(false)
  const [activeSection, setActiveSection] = useState('quick-index')
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1 lg:ml-[260px] min-w-0">
        ...
        <div className="sticky top-0 z-30 bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-3 lg:hidden">
          ...
          <span className="text-sm font-bold text-gray-800 truncate">학종 실무 핸드북</span>
        </div>
        ...
        <div className="hidden lg:flex sticky top-0 z-30 bg-white/95 backdrop-blur border-b border-gray-200 px-12 py-2.5 text-[12px] text-gray-500 items-center justify-between">
          <div className="flex items-center gap-2">
            <span>학종 실무 핸드북</span>
            <span>›</span>
            <span>Top 10 막힘 사전 · 9 카테고리 · 5 반복 함정</span>
          </div>
          <span className="font-semibold text-rose-600">막혔을 때 30초 안에 답 찾기</span>
        </div>
